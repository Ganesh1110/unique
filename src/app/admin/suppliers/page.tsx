...
import { Package, Plus, ArrowLeft, Phone, MapPin, CheckCircle2, Clock, Truck, MessageCircle } from 'lucide-react';
...
export default function AdminSuppliersPage() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-cream-50">
      ...
      <header className="section-sm bg-white border-b border-neutral-200">
        <div className="container space-y-4">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <span className="overline text-gold-600 block mb-1">Supply Chain &amp; Saree Restock</span>
              <h1 className="font-heading text-display-md text-neutral-950">Master Weaver Purchase Orders</h1>
            </div>
            <button
              onClick={() => setShowNewModal(true)}
              className="btn-primary text-body-xs font-bold uppercase tracking-wider py-2.5 px-5 bg-[#E60012] hover:bg-red-700 text-white min-h-[44px] inline-flex items-center gap-2 self-start sm:self-auto"
            >
        ...
        <div className="container">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
            <div className="card p-5 space-y-1 border-l-4 border-l-gold-500">
              <span className="text-caption text-neutral-500 font-semibold uppercase">Active Purchase Orders</span>
              <p className="font-heading text-display-sm text-neutral-950 tabular-nums">{orders.length} orders</p>
              <p className="text-caption text-neutral-500">Across Kanchipuram &amp; Varanasi</p>
            </div>

            <div className="card p-5 space-y-1 border-l-4 border-l-emerald-500">
              <span className="text-caption text-neutral-500 font-semibold uppercase">Total Restock Capital</span>
              <p className="font-heading text-display-sm text-neutral-950 tabular-nums">{formatMoney(totalCommitted, 'INR')}</p>
              <p className="text-caption text-emerald-700 font-medium">Committed weaver PO value</p>
            </div>

            <div className="card p-5 space-y-1 border-l-4 border-l-sky-500">
              <span className="text-caption text-neutral-500 font-semibold uppercase">Advance Payments Paid</span>
              <p className="font-heading text-display-sm text-neutral-950 tabular-nums">{formatMoney(totalAdvance, 'INR')}</p>
              <p className="text-caption text-neutral-500">Balance: {formatMoney(totalCommitted - totalAdvance, 'INR')}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      {/* Orders List */}
      <section className="section" aria-label="Weaver purchase orders">
        <div className="container space-y-6 max-w-5xl">
          <div className="divide-y divide-neutral-200 bg-white rounded-xl shadow-sm border border-neutral-200 overflow-hidden">
            {orders.map((po) => (
              <div key={po.id} className="p-6 space-y-4 hover:bg-neutral-50/50 transition-colors">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-neutral-100 pb-3">
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="font-mono text-body-xs font-bold text-neutral-950 bg-neutral-100 px-2 py-0.5 rounded">{po.id}</span>
                      <span className="text-caption text-neutral-500 flex items-center gap-1">
                        <MapPin className="h-3.5 w-3.5 text-neutral-400" /> {po.location}
                      </span>
                    </div>
                  ...
                  <div className="flex items-center gap-3">
                    <span
                      className={`inline-flex items-center gap-1.5 text-caption font-extrabold uppercase tracking-wider px-3 py-1 rounded-full ${
                    ...
                    >
                      ...
                      {po.status === 'Received & QC Passed' && <CheckCircle2 className="h-3.5 w-3.5" />}
                      {po.status}
                    </span>
                  </div>
                </div>
                ...
                {/* Details */}
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-body-xs bg-neutral-50 p-4 rounded-lg border border-neutral-200">
                  <div className="sm:col-span-2 space-y-1">
                    <span className="text-[10px] font-bold uppercase tracking-widest text-neutral-400 block">Saree &amp; Weave Details</span>
                    <p className="font-semibold text-neutral-950">{po.sareeDescription}</p>
                    <p className="text-caption text-neutral-500">Expected Batch Arrival: {po.expectedDate}</p>
                  </div>

                  <div className="space-y-1 sm:border-l border-neutral-200 sm:pl-4">
                    <span className="text-[10px] font-bold uppercase tracking-widest text-neutral-400 block">Payment Financials</span>
                    <p className="font-bold text-neutral-950">Total PO: {formatMoney(po.totalCost, 'INR')}</p>
                    <p className="text-emerald-700 font-semibold">Advance Paid: {formatMoney(po.advancePaid, 'INR')}</p>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 pt-2">
                  <a
                    ...
                    className="btn-secondary text-body-xs font-bold flex items-center gap-1.5 border-emerald-600 text-emerald-800 bg-emerald-50 hover:bg-emerald-100"
                  >
                  ...
                  <div className="flex gap-2">
                    {po.status === 'Weaving In Progress' && (
                      <button
                        ...
                        className="btn-secondary text-body-xs py-1.5 px-3"
                      >
                        Mark In Transit
                      </button>
                    )}
                    {po.status === 'In Transit' && (
                      <button
                        ...
                        className="btn-primary text-body-xs py-1.5 px-3 bg-emerald-600 hover:bg-emerald-700 text-white"
                      >
      ...
      {showNewModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-neutral-950/70 backdrop-blur-sm">
          <div className="bg-white p-6 sm:p-8 rounded-2xl max-w-lg w-full space-y-4 border border-neutral-300 shadow-2xl">
            <h2 className="font-heading text-heading-md text-neutral-950">Create New Weaver Purchase Order</h2>
            <form onSubmit={handleCreateOrder} className="space-y-3 text-body-xs">
              <div>
                <label className="label text-[10px]">Master Weaver / Guild Name</label>
                <input
                  type="text"
                  required
                  ...
                  className="input py-2 text-body-xs"
                />
              </div>

              <div>
                <label className="label text-[10px]">Weaving Guild Location</label>
                <select
                  value={location}
              ...
              <div>
                <label className="label text-[10px]">Saree Description &amp; Weave Batch</label>
                <textarea
                  required
                  rows={2}
                  ...
                  className="input py-2 text-body-xs min-h-[60px]"
                />
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="label text-[10px]">Total PO Cost (₹)</label>
                  <input
                    type="number"
                    required
                    value={cost}
                ...
                <div>
                  <label className="label text-[10px]">Advance Paid (₹)</label>
                  <input
                    type="number"
                    required
                    value={advance}
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-neutral-200">
                ...
                <button type="submit" className="btn-primary bg-[#E60012] hover:bg-red-700 text-white">

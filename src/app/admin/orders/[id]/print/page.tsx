...
export default function OrderPrintInvoicePage() {
  ...
  const hasValidAddress = Boolean(
    order.address && (order.address.addressLine?.trim() || order.address.city?.trim() || order.address.pincode?.trim())
  );

  return (
    <div className="min-h-screen bg-neutral-100 p-4 sm:p-8 print:p-0 print:m-0 print:bg-white text-neutral-950 font-sans">
      <style jsx global>{`
        @page {
          size: A4 portrait;
          margin: 10mm;
      ...
      <div className="max-w-3xl mx-auto bg-white p-8 sm:p-10 print:p-6 rounded-xl shadow-xl print:shadow-none border border-neutral-300 print:border-none space-y-6 print:space-y-4 print:w-full print:max-w-none">
        {/* Invoice Header */}
        <div className="flex items-start justify-between border-b-2 border-neutral-950 pb-4 print:pb-3">
          <div>
            <h1 className="font-heading text-display-md print:text-2xl font-extrabold tracking-tight text-neutral-950">AURA</h1>
            ...
            <p className="text-caption text-neutral-600 mt-1.5">GSTIN: 27AABCA1234F1Z0 · FSSAI / Textile License: TXT-998822</p>
          ...
          <div className="text-right space-y-1">
            ...
            <p className="font-heading text-heading-sm print:text-base font-bold text-neutral-950 mt-1">Invoice #{cleanOrderNumber}</p>
        ...
        <div className="grid grid-cols-2 gap-6 text-body-xs bg-neutral-50 print:bg-neutral-50/60 p-4 print:p-3 rounded-xl border border-neutral-200">
          <div>
            <span className="text-[10px] font-extrabold uppercase tracking-widest text-neutral-400 block mb-1">Billed &amp; Shipped To</span>
            <p className="font-bold text-neutral-950 text-body-sm">{order.name || 'Valued Customer'}</p>
            {hasValidAddress && order.address ? (
              <p className="text-neutral-700 leading-snug mt-1">
                {order.address.addressLine && <>{order.address.addressLine}<br /></>}
                {[order.address.city, order.address.state].filter(Boolean).join(', ')}
                {order.address.pincode ? ` - ${order.address.pincode}` : ''}
              </p>
            ) : (
              <p className="text-neutral-500 italic text-caption mt-1">Direct Checkout / Express Counter Delivery</p>
            )}
            {order.email && <p className="text-neutral-600 mt-1.5 flex items-center gap-1.5"><Mail className="h-3 w-3 text-neutral-400" /> {order.email}</p>}
            {order.phone && <p className="text-neutral-600 flex items-center gap-1.5"><Phone className="h-3 w-3 text-neutral-400" /> {order.phone}</p>}
          </div>
          ...
          <div className="space-y-1.5 border-l border-neutral-200 pl-4">
            ...
            <p className="text-neutral-700"><strong>AWB Airway Bill:</strong> AURA-{cleanOrderNumber}-IND</p>
          ...
          <table className="w-full text-left border-collapse text-body-xs">
            ...
            <tbody className="divide-y divide-neutral-200">
              {order.lineItems.map((item, idx) => {
                const unitPrice = item.price && item.price > 0 ? item.price : Math.round(order.total / (order.lineItems.length || 1));
                const lineTotal = unitPrice * (item.quantity || 1);
                return (
                  <tr key={idx}>
                    <td className="py-2 px-3 font-semibold text-neutral-950">
                      {item.title}
                    ...
                    <td className="py-2 px-3 text-center text-neutral-500 font-mono text-[11px]">5007 (Silk)</td>
                    <td className="py-2 px-3 text-center font-bold text-neutral-950">{item.quantity}</td>
                    <td className="py-2 px-3 text-right text-neutral-700 tabular-nums">{formatMoney(unitPrice, order.currencyCode)}</td>
                    <td className="py-2 px-3 text-right font-bold text-neutral-950 tabular-nums">{formatMoney(lineTotal, order.currencyCode)}</td>
                  </tr>
                );
              })}
            </tbody>
        ...
        <div className="flex justify-end pt-2 border-t border-neutral-200">
          <div className="w-64 space-y-1.5 text-body-xs">
            ...
            <div className="border-t-2 border-neutral-950 pt-1.5 flex justify-between text-body font-bold text-neutral-950">
        ...
        <div className="border-t border-neutral-300 pt-4 text-[10px] text-neutral-500 space-y-1">
